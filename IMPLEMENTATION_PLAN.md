...
- [ ] **Fix 27 typecheck errors in test files** — `npx tsc --noEmit` shows errors in admin.service.spec, reviews/comments/complaints.service.spec (userVote type), and e2e tests (type assertions). Production code is clean.
- [ ] **Fix lint errors in test files** — 556 problems (354 errors, 202 warnings), primarily `@typescript-eslint` strict-mode violations in test/e2e and test/helpers.
